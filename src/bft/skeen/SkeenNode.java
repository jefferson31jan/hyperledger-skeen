...
public class SkeenNode {
    ...
    public synchronized void onReceiveStart(SkeenMsg msg) {
        ...
        // registrar destinos se ainda não conhecemos essa mensagem
    ...
    public synchronized void onReceiveLocalTs(SkeenMsg msg) {
        ...
        if (dest == null) {
            logger.warn("[{}] destMap nulo para msgId={}", shardId, mid);
            return;
        }
        ...
        long maxTs = localTsBuffer.get(mid).values().stream()
    ...
    public synchronized void onReceiveAck(SkeenMsg msg) {
        ...
        ackBuffer.computeIfAbsent(mid, k -> ConcurrentHashMap.newKeySet())
                 .add(msg.fromShard);
